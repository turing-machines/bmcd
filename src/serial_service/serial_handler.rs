...
use super::serial_channel::SerialChannel;
...
pub struct Handler {
    ...
    writer: Option<WeakSender<BytesMut>>,
}
...
impl Handler {
    ...
    /// Opens a stream which can be used to read and write data with.
    ///
    /// # Returns
    ///
    /// * `SerialError::NotStarted` when [`Self::run`] was not called
    /// successfully
    pub fn open_channel(&self) -> Result<impl AsyncRead + AsyncWrite, SerialError> {
        let Some((read_sender, write_sender)) = &self.worker_context else {
            return Err(SerialError::NotStarted);
        };

        Ok(SerialChannel::new(
            read_sender.subscribe(),
            write_sender.clone(),
        ))
    }
    ...
    /// Time complexity: O(N)
    pub async fn read_whole_buffer(&self) -> Result<BytesMut, SerialError> {
    ...
    pub async fn write(&self, bytes: BytesMut) -> Result<(), SerialError> {
        let writer = self
            .writer
            .as_ref()
